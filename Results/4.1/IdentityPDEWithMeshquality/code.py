...
while iteration < maxIter:
    ...
    objectives_meshquality.append(assemble(k_meshquality*objective_mesh_quality))
    while (not lineSearchSuccessful) and (alpha > alphaMin):
        ...
        if (trialJ <= J - sigma * alpha * normShapeGradient2):
            ...
            # Check if all signed cell volumes are positive after mesh update
            for cell in cells(mesh):
                cell_volume = cell.volume()   
                if cell_volume <= 0:
                    print("Warning: Mesh contains cells with non-positive signed volume after update.")
                    # Optionally, you can break or handle this case as needed
        else:
            alpha = beta * alpha
...
ax1.plot(objectives_main, label="Main Objective", color="tab:blue", linestyle=":")
ax1.plot(objectives_meshquality, label="Mesh Quality Objective", color="tab:blue", linestyle="--")
...
ax1_lin.plot(objectives_main, label="Main Objective", color="tab:blue", linestyle=":")
ax1_lin.plot(objectives_meshquality, label="Mesh Quality Objective", color="tab:blue", linestyle="--")
ax1_lin.legend(fontsize=12)
...
# plot rel_changes 
plt.figure()
plt.plot( [i+1 for i in range(len(rel_changes))], rel_changes, label="Relative Change in Objective")
...
plt.figure()
plt.plot(objectives_main, label="Main Objective")
plt.plot(objectives_meshquality, label="Mesh Quality Objective")
plt.xlabel("Iteration", fontsize=12)
plt.legend(fontsize=12)
